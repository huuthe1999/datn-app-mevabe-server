...
const StandardGrowNote = require("../models/standard-height-note");
...
const updateChildGrowNoteByID = async (req, res, next) => {
    ...
    let note;
    try {
        note = await HeightNote.findById(gId);
    } catch (err) {
...
const deleteChildGrowNoteByID = async (req, res, next) => {
    ...
    let note;
    try {
        note = await HeightNote.findByIdAndDelete(gId);
    } catch (err) {
